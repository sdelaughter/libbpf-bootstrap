...
const char *argp_program_version = "egress_prover 0.0";
const char *argp_program_bug_address = "<bpf@vger.kernel.org>";
const char argp_program_doc[] =
"BPF egress_prover demo application.\n"
"\n"
"It prints the size of received packets\n"
"\n"
"USAGE: ./egress_prover [-i <interface>] [-v]\n";
...
int main(int argc, char **argv)
{
	...
	err = egress_prover_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load and verify BPF skeleton\n");
		goto cleanup;
	}

	// egress_prover_bpf__set_type(skel, BPF_PROG_TYPE_XDP);

	/* Attach tracepoints */
	// err = egress_prover_bpf__attach(skel);
	// if (err) {
	...
	egress_prover_bpf__detach(skel);
	egress_prover_bpf__destroy(skel);
