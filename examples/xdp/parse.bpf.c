  ...
  if ((void *)eth + sizeof(*eth) <= data_end) {
    ...
    if (eth->h_proto == htons(ETH_P_IP)) {
      ...
      if ((void *)ip + sizeof(*ip) <= data_end) {
        ...
        // e->ip_len = ip->tot_len;
        // e->ip_id = ntohs(ip->id);
        // e->flag_df = ntohs(ip->frag_off & 0x4000);
        // e->flag_mf = ntohs(ip->frag_off & 0x2000);
        // e->ip_offset = ntohs(ip->frag_off & 0x1FFF);
        e->ttl = ip->ttl;
        e->ip_protocol = ip->protocol;
        // e->ip_checksum = ip->check;
        e->ip_saddr = ntohs(ip->saddr);
        e->ip_daddr = ntohs(ip->daddr);
        ...
        if (ip->protocol == IPPROTO_UDP) {
          ...
          if ((void *)tcp + sizeof(*tcp) <= data_end) {
            ...
            // e->tcp_seq = ntohs(tcp->seq);
            // e->tcp_ack_seq = ntohs(tcp->ack_seq);
            // e->tcp_doff = tcp->doff;
            // e->tcp_res1 = tcp->res1;
            // e->tcp_cwr = tcp->cwr;
            // e->tcp_ece = tcp->ece;
