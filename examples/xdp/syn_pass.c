...
const char *argp_program_version = "syn_pass 0.0";
const char *argp_program_bug_address = "<bpf@vger.kernel.org>";
const char argp_program_doc[] =
"BPF syn_pass demo application.\n"
"\n"
"It prints the size of received packets\n"
"\n"
"USAGE: ./syn_pass [-i <interface>] [-v]\n";
...
int main(int argc, char **argv)
{
	...
	err = syn_pass_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load and verify BPF skeleton\n");
		goto cleanup;
	}

	// syn_pass_bpf__set_type(skel, BPF_PROG_TYPE_XDP);

	/* Attach tracepoints */
	// err = syn_pass_bpf__attach(skel);
	// if (err) {
	...
	syn_pass_bpf__detach(skel);
	syn_pass_bpf__destroy(skel);
