...
const char *argp_program_version = "icmp_pad 0.0";
const char *argp_program_bug_address = "<bpf@vger.kernel.org>";
const char argp_program_doc[] =
"BPF icmp_pad demo application.\n"
"\n"
"It prints the size of received packets\n"
"\n"
"USAGE: ./icmp_pad [-i <interface>] [-v]\n";
...
int main(int argc, char **argv)
{
	...
	err = icmp_pad_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load and verify BPF skeleton\n");
		goto cleanup;
	}

	// icmp_pad_bpf__set_type(skel, BPF_PROG_TYPE_XDP);

	/* Attach tracepoints */
	// err = icmp_pad_bpf__attach(skel);
	// if (err) {
...
cleanup:
	...
	icmp_pad_bpf__detach(skel);
	icmp_pad_bpf__destroy(skel);
